...
#   /26225005_origin_export_with_d_wave.xlsx
...
xlsx_path   <- Sys.getenv(
  "ZEUS_VALIDATION_C0_XLSX",
  unset = file.path("temp_file", "26225005_origin_export_with_d_wave.xlsx")
)
...
if (!all(protocol_validation$passed)) {
  stop("C0 protocol label validation failed.", call. = FALSE)
}


# Compare sweep → stim_index mapping for first and last few sweeps
...
if (!all(sweep_validation$passed)) {
  stop("C0 sweep mapping validation failed.", call. = FALSE)
}
...
if (nrow(wave_compare) == 0L) {
  ...
  stop("C0 trace response validation failed: no matched rows.", call. = FALSE)
} else {
  ...
  wave_raw_validation <- zeus_validate_response_agreement(
    ...
    label = "C0 unsmoothed trace response",
    error = FALSE
  )
  ...
  wave_smoothed_validation <- zeus_validate_response_agreement(
    ...
    label = "C0 smoothed trace response",
    error = FALSE
  )

  if (!all(wave_smoothed_validation$passed)) {
    print(wave_smoothed_validation)
    stop("C0 trace response validation failed.", call. = FALSE)
  }
...
if (exists("wave_raw_agreement", inherits = FALSE)) {
  results_c0$wave_raw_agreement <- wave_raw_agreement
  results_c0$wave_smoothed_agreement <- wave_smoothed_agreement
  results_c0$wave_raw_validation <- wave_raw_validation
  results_c0$wave_smoothed_validation <- wave_smoothed_validation
}

results_c0$protocol_validation <- protocol_validation
results_c0$sweep_validation <- sweep_validation
