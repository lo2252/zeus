...
measure_trace_window <- function(trace_df,
  ...
  if (nrow(base_df) == 0L || nrow(resp_df) == 0L || nrow(awave_df) == 0L) {
    return(tibble::tibble(
      ...
      amp_mv = NA_real_,
      awave_mv = NA_real_,
      awave_time_ms = NA_real_,
      trough_time_ms = NA_real_,
      peak_time_ms = NA_real_,
      awave_time_poststim_ms = NA_real_,
  ...
  awave_time_current <- awave_row[[time_col]][[1]]
  ...
  tibble::tibble(
    ...
    amp_mv = amp_mv,
    awave_mv = awave_mv,
    awave_time_ms = awave_time_current,
    trough_time_ms = trough_time_current,
    peak_time_ms = peak_time_current,
    awave_time_poststim_ms = if (use_relative) awave_time_current else awave_time_current - stimulus_onset_ms,
